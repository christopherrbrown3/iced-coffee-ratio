...
import { getBrewer, getTimedBrewSeconds, type BrewResult, type BrewSettings } from '../domain/brew'
import { formatTime, useBrewTimer } from '../hooks/useBrewTimer'
...
function getTimedSteps(settings: BrewSettings, result: BrewResult, timedSeconds: number): TimedStep[] {
  if (settings.method === 'flash') {
    return [
      { atSeconds: 0, eyebrow: 'Bloom', title: 'Wake up the coffee', instruction: `Start the timer and bloom with ${scaledPour(result.hotWaterMl, 30)} mL, wetting every ground.` },
      { atSeconds: 30, eyebrow: 'Pour', title: 'Circle to the first mark', instruction: `Pour in gentle circles until the scale reaches ${scaledPour(result.hotWaterMl, 100)} mL.` },
      { atSeconds: 60, eyebrow: 'Pour', title: 'Build the brew', instruction: `When the bed drops about 1 cm, pour in circles to ${scaledPour(result.hotWaterMl, 200)} mL.` },
...
export function TimerView({ settings, result, onClose }: TimerViewProps) {
  ...
  const timeDisplay = timer.phase === 'release' || timer.phase === 'complete'
    ...
    <div className={`timer-view timer-view--${tone}`}>
      ...
      <main id="timer-main" className="timer-main">
        <section className="timer-stage">
          <div className="timer-stage__copy" aria-live="polite" aria-atomic="true">
            <p className="comic-kicker">{displayCopy.eyebrow}</p>
            <h1 ref={headingRef} tabIndex={-1}>{displayCopy.title}</h1>
          </div>

          <div className="timer-clock" role="timer" aria-live="off" aria-label={`${timeDisplay} ${timer.phase === 'release' ? 'elapsed' : 'remaining'}`}>
            {timeDisplay}
          </div>
        ...
        <aside className="timer-recipe" aria-label="Current recipe">
          <img src={`${assetBase}graphics/ice-mascot-pointing.png`} alt="Cheerful illustrated ice cube pointing toward the brew" />
          <dl>
